...
def run_search():
    data = pd.read_csv('data/basinCount/secondLevelSearch.csv')
    ...
    # dictionary of lists
    basins = {'basin_codes': basin_code_array, 'basin_counts': basin_count_array}
    print("                  ")
    print("                  ")
    print(basins)

    df = pd.DataFrame(basins)
    print(df)
    df.to_csv('/data/basin_counts.csv')
