...
def single_basin_search():
    ...
    for index, row in status_data.iterrows():
        print("New row and the result count is ", result_count)
        basin = row['basin']
        key = row['key']
        ...
        if finished != 1:
            ...
            if result_count < 1000 and result_count != None:
...
def bulk_download(index, result_count, start_count, stop_count):
    ...
    if result_count > stop_count or run_last_search == True:
        print("Run the search ", "start_count ", start_count, " stop_count ",  end_count)
