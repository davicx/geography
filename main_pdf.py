...
#STEP 7: Download PDF
def download_bulk_pdf(basin_code, result_count_raw):
    result_count = int(result_count_raw)

    print("STEP 7: Download PDF Files") 
    ...
    for i in range(0, result_count, max_downloads):
        ...
        print(min, max)
        download_pdf(min, max)
        time.sleep(2)


def download_pdf(min_raw, max_raw):
    min = str(min_raw)
    max = str(max_raw)
    print("Starting Downloads: Excel files for from ", min, " to ", max)
