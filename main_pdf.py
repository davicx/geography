...
#SETUP: User and Basin Information Setup (TO DO: Set this for yourself and the current basin)
external_user = True
basin_code = "aral"
master_user = "david"
...
def single_basin_search():
    ...
    for index, row in status_data.iterrows():
        ...
        #ROW: Start Single Row
        if finished != 1:

            #BASE: Run Base Search
            if set_date == 1:
                print("BASE: Run Base Search")
                result_count = run_base_search(search_link, start_date, end_date)

            #Do a bulk download if the results are under 1000
            if result_count < 1000 and result_count != 999999:
                bulk_download(index, result_count, start_count, stop_count)

            else:
                update_status_over_limit(index, result_count)
                print("Mark Over 1000")

        #ROW: Row id done so skip
        else:     
            print("The basin ", basin, " from ", start_date, " to ", end_date, " is already done so we are skipping")
            time.sleep(1)

def run_base_search(search_link, start_date, end_date):

    #STEP 1: Base Search 
    base_search()

    #STEP 2: Set Date Function
...
def bulk_download(index, result_count, start_count, stop_count):
    run_last_search = False
    end_count = min(result_count, stop_count)
    #print("end_count", end_count, " result_count ", result_count, " stop_count ", stop_count)
    
    if result_count > start_count and result_count < stop_count:
        run_last_search = True

    if result_count > stop_count or run_last_search == True:
        print("Run the search ", "start_count ", start_count, " stop_count ",   )
        time.sleep(2)

        final_download_outcome = True
     
        #final_download_outcome = download_pdf(index, result_count, start_count, stop_count)
        final_download_outcome = download_pdf(index, result_count, start_count, end_count)

        if final_download_outcome == True:
            update_status_success(index, result_count, start_count, stop_count)
           
    else:
        print("We don't need but mark done ", stop_count)
        update_status_success(index, result_count, start_count, stop_count)
          
    print("bulk download ", result_count, " start ", start_count, " stop ", stop_count)
    time.sleep(1)
...
def download_pdf(index, result_count, start_count, stop_count):
    ...
    min = str(start_count)
    max = str(stop_count)
    total_pdf_downloads = str(total_pdf_downloads_int)
    ...
    original_file_name = "Files (" + total_pdf_downloads + ").PDF"
    download_file_name = "ResultsList_" + basin_code + "_202207_" + min + "_" + max + "_PDF"

    download_outcome = move_rename_file(original_file_name, download_folder_temp, download_file_name, download_folder)
...
def update_status_success(index, result_count, start_count, stop_count):
    ...
    status_data.loc[index, ['file_name']] = [download_file_name]
